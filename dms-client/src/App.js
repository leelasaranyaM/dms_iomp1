...
function App() {
  ...
  // --- Restore admin login from localStorage on page load ---
  useEffect(() => {
  ...
  // --- Fetch disaster events for dashboard / map ---
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get(DISASTER_API_URL);
        // Backend may return an array or {features: [...]}
  ...
  const activeAlerts = events.filter(
    e => e.properties?.severity === 'Severe' || e.properties?.severity === 'Extreme'
  ).length;
  const criticalEvents = events.filter(
    e => e.properties?.severity === 'Extreme'
  ).length;
  ...
  const handlePlanRoute = (source, destination, alerts) => {
    ...
    alerts.forEach(alert => {
      const location = alert.manualAddress || (
        alert.geolocation 
          ? `${alert.disasterType} at ${alert.geolocation.lat}, ${alert.geolocation.lon}` 
          : alert.disasterType
      );
    ...
    <Container className="my-5">
      ...
      {loading ? (
        <div className="text-center my-5">
          ...
          <p className="text-info mt-2">Loading real-time data...</p>
        </div>
      ) : (
        <>
          {error && (
          ...
          <Row xs={1} md={2} lg={4} className="g-4 mb-5">
            <Col>
              <StatusCard
                ...
                value={activeAlerts}
                theme="alert"
                description="High-priority events needing attention."
              />
            </Col>
            <Col>
              <StatusCard
                title="Critical Events (Extreme)"
                value={criticalEvents}
                theme="critical"
                description="Events with highest potential impact."
              />
            </Col>
            <Col>
              <StatusCard
                title="Safety Guides"
                value={safetyGuidesCount}
                theme="success"
                description="Educational guides available for download."
              />
            </Col>
            <Col>
              <StatusCard
                title="Open Aid Requests"
                value={openAidRequests}
                theme="info"
                description="Requests for food, water, or medical help."
              />
            </Col>
          </Row>
            ...
            <Col lg={7} className="p-0"> 
              <Card className="shadow-sm h-100">
                <Card.Header as="h5" className="bg-primary text-white">
                  Live Map Visualization (India Hazards)
                </Card.Header>
            ...
            <Col lg={5} className="p-0"> 
              <Card className="shadow-sm h-100">
                <Card.Header as="h5" className="bg-secondary text-white">
                  Latest Event Details ({totalEvents} Found)
                </Card.Header>
      ...
      <div className="hero-section text-center py-5 mb-5 bg-light shadow-sm">
        <Container>
          <h1 className="display-4 text-primary">
            Your Local Disaster Safety & Alert Hub 🇮🇳
          </h1>
          <p className="lead text-muted">
            We provide <strong>real-time alerts</strong> for all hazards in India,{' '}
            <strong>safety guidance</strong>, and <strong>coordinate aid</strong> during emergencies.
          </p>
          <Button
            variant="danger"
            size="lg"
            onClick={() =>
              window.scrollTo({
                top: document.getElementById('alerts')?.offsetTop || 0,
                behavior: 'smooth',
              })
            }
          >
      ...
      <Container>
        ...
        <section id="alerts" className="my-5 py-3">
          <LiveAlertsSection
            ...
            onAlertsFetched={setActiveHelpRequests}
          />
        </section>
  ...
  const renderAdminPortal = () => (
    <Container className="my-5 admin-portal-wrapper">
      ...
      <p className="lead text-center text-muted mb-5">
        Incident Management for {adminToken}
      </p>
